...
"$DANUS_PY" -I -B -c 'from danus._mcp import FastMCP' 2>/dev/null && ok "python dep: mcp" || no "python dep: mcp missing or incompatible"
(cd / && "$DANUS_PY" -I -B -c 'import danus' 2>/dev/null) && ok "python pkg: danus (importable from any cwd)" || no "danus package not installed in the venv — worker MCP gateway and bin/ wrappers fail off the repo root (run bootstrap.sh)"
"$DANUS_PY" -I -B -c 'import fastapi, uvicorn, pydantic' 2>/dev/null && ok "python deps: fastapi/uvicorn/pydantic" || no "verify-service deps missing"
"$DANUS_PY" -I -B -c 'import openai' 2>/dev/null && ok "python dep: openai (gpt_pro consult)" || wn "openai missing (gpt_pro consult disabled)"
"$DANUS_PY" -I -B -c 'import anthropic' 2>/dev/null && ok "python dep: anthropic (claude_api consult)" || wn "anthropic missing (claude_api consult disabled)"
